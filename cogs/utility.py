...
class ModmailHelpCommand(commands.HelpCommand):
    async def format_cog_help(self, cog, *, no_cog=False):
        ...
        for format_ in formats:
            ...
            embed.add_field(name=_("Commands"), value=format_ or _("No commands."))

            continued = " " + _("(Continued)") if embeds else ""
            name = (
                cog.qualified_name + " - " + _("Help")
                if not no_cog
                else _("Miscellaneous Commands")
            )
    ...
    async def send_command_help(self, command):
        ...
        if topic is not None:
            topic[0].set_footer(text=_("Permission level: {level}").format(level=topic[1]))
    ...
    async def send_group_help(self, group):
        ...
        if topic is None:
            return
        embed = topic[0]
        embed.add_field(name=_("Permission Level"), value=topic[1], inline=False)
    ...
    async def send_error_message(self, error):
        ...
        if val is not None:
            return await self.get_destination().send(
                escape_mentions(_("**`{command}` is a snippet, content:**\n\n{val}").format(command=command, val=val))
            )
        ...
        if val is not None:
            ...
            if len(values) == 1:
                embed = discord.Embed(
                    ...
                    color=self.context.bot.main_color,
                    description=_("`{command}` points to `{value}`.").format(command=command, value=escape_markdown(values[0])),
                )
            else:
                embed = discord.Embed(
                    title=_("{command} is an alias.").format(command=command),
            ...
            embed.set_footer(
                text=_('Type "{prefix}{command} alias" for more '
                       "details on aliases.").format(prefix=self.clean_prefix, command=self.command_attrs["name"])
            )
        ...
        embed.set_footer(text=_('Command/Category "{command}" not found.').format(command=command))
...
class Utility(commands.Cog):
    ...
    async def about(self, ctx):
        ...
        embed.add_field(name=_("Latency"), value=f"{self.bot.latency * 1000:.2f} ms")
        embed.add_field(name=_("Version"), value=f"`{self.bot.version}`")
        embed.add_field(name=_("Authors"), value="`kyb3r`, `Taki`, `fourjr`")
        ...
        if self.bot.version.is_prerelease:
            ...
            footer = _("You are up to date with the latest version.")
    ...
    async def debug(self, ctx):
        ...
        if not logs:
            embed = discord.Embed(
                ...
                title=_("Debug Logs:"),
                description=_("You don't have any logs at the moment."),
            )
            embed.set_footer(text=_("Go to Heroku to see your logs."))
        ...
        embed.set_footer(text=_("Debug logs - Navigate using the reactions below."))
    ...
    async def debug_hastebin(self, ctx):
        ...
        try:
            ...
            embed = discord.Embed(
                title="Debug Logs",
                color=self.bot.main_color,
                description=_("Something's wrong. "
    ...
    async def debug_clear(self, ctx):
        ...
        await ctx.send(
            embed=discord.Embed(
                color=self.bot.main_color, description=_("Cached logs are now cleared.")
            )
        )
    ...
    async def activity(self, ctx, activity_type: str.lower, *, message: str = ""):
        ...
        if activity_type == "clear":
            ...
            embed = discord.Embed(title=_("Activity Removed"), color=self.bot.main_color)
            return await ctx.send(embed=embed)
        ...
        msg = _("Activity set to: {name} ").format(name=activity.type.name.capitalize())
        if activity.type == ActivityType.listening:
            msg += _("to {name}.").format(name=activity.name)
        else:
            msg += f"{activity.name}."

        embed = discord.Embed(
            title=_("Activity Changed"), description=msg, color=self.bot.main_color
        )
        return await ctx.send(embed=embed)
    ...
    async def status(self, ctx, *, status_type: str.lower):
        ...
        if status_type == "clear":
            ...
            await self.bot.config.update()
            await self.set_presence()
            embed = discord.Embed(title=_("Status Removed"), color=self.bot.main_color)
            return await ctx.send(embed=embed)
        ...
        msg = _("Status set to: {value}.").format(value=status.value)
        embed = discord.Embed(
            title=_("Status Changed"), description=msg, color=self.bot.main_color
        )
        return await ctx.send(embed=embed)
    ...
    async def prefix(self, ctx, *, prefix=None):
        ...
        embed = discord.Embed(
            title=_("Current prefix"), color=self.bot.main_color, description=f"{current}"
        )

        if prefix is None:
            await ctx.send(embed=embed)
        else:
            embed.title = _("Changed prefix!")
            embed.description = _("Set prefix to `{prefix}`").format(prefix=prefix)
    ...
    async def config_options(self, ctx):
        ...
        for names in zip_longest(*(iter(sorted(self.bot.config.public_keys)),) * 15):
            ...
            embed = discord.Embed(
                title=_("Available configuration keys:"),
                color=self.bot.main_color,
    ...
    async def config_set(self, ctx, key: str.lower, *, value: str):
        ...
        if key in keys:
            ...
            embed.add_field(name=_("Valid keys"), value=", ".join(valid_keys))
    ...
    async def config_remove(self, ctx, *, key: str.lower):
        ...
        if key in keys:
            ...
            embed = discord.Embed(
                title=_("Success"),
                color=self.bot.main_color,
                description=_("`{key}` had been reset to default.").format(key=key),
            )
        else:
            embed = discord.Embed(
                title=_("Error"),
    ...
    async def config_help(self, ctx, key: str.lower = None):
        ...
        if key is not None and key not in config_help:
            embed = discord.Embed(
                title=_("Error"),
                color=self.bot.error_color,
                description=_("No help details found for `{key}`.").format(key=key),
            )
            return await ctx.send(embed=embed)
        ...
        for i, (current_key, info) in enumerate(config_help.items()):
            ...
            if info["examples"]:
                ...
                embed.add_field(name=_("Example(s):"), value=example_text, inline=False)

            note_text = ""
            for note in info["notes"]:
                note_text += f"- {fmt(note)}\n"
            if note_text:
                embed.add_field(name=_("Note(s):"), value=note_text, inline=False)
    ...
    async def alias(self, ctx, *, name: str.lower = None):
        ...
        if name is not None:
            ...
            if len(values) == 1:
                embed = discord.Embed(
                    color=self.bot.main_color,
                    description=_("`{name}` points to `{escape_markdown(values[0])}`.").format(name=name),
                )
            else:
                embed = discord.Embed(
                    color=self.bot.main_color,
                    description=_("**`{name}` points to the following steps:**").format(name=name),
                )
                for i, val in enumerate(values, start=1):
                    embed.description += f"\n{i}: {escape_markdown(val)}"
        ...
        for i, names in enumerate(zip_longest(*(iter(sorted(self.bot.aliases)),) * 15)):
            ...
            embed.set_author(name=_("Command Aliases"), icon_url=ctx.guild.icon_url)
            embeds.append(embed)
    ...
    async def alias_add(self, ctx, name: str.lower, *, value):
        ...
        if self.bot.get_command(name):
            ...
            embed = discord.Embed(
                title=_("Error"),
                color=self.bot.error_color,
                description=_("Alias names cannot be longer than 120 characters."),
            )
        ...
        if not values:
            embed = discord.Embed(
                title=_("Error"),
                color=self.bot.error_color,
                description=_("Invalid multi-step alias, try wrapping each steps in quotes."),
            )
            embed.set_footer(text=_('See "{prefix}alias add" for more details.').format(prefix=self.bot.prefix))
            return await ctx.send(embed=embed)
        ...
        if len(values) == 1:
            ...
            if not self.bot.get_command(linked_command):
                ...
                if alias_command is not None:
                    ...
                    embed = discord.Embed(
                        title=_("Error"),
                        color=self.bot.error_color,
                        description=_("The command you are attempting to point "
            ...
            embed = discord.Embed(
                title=_("Added alias"),
                color=self.bot.main_color,
                description=_('`{name}` points to: "{value}".').format(name=name, value=values[0]),
            )
            ...
            embed = discord.Embed(
                title=_("Added alias"),
                color=self.bot.main_color,
                description=_("`{name}` now points to the following steps:").format(name=name),
            )
                ...
                if not self.bot.get_command(linked_command):
                    ...
                    if alias_command is not None:
                        ...
                        embed = discord.Embed(
                            ...
                            color=self.bot.error_color,
                            description=_("The command you are attempting to point "
                                          "to n step {number} does not exist: `{command}`.").format(number=i, command=linked_command),
                        )
                        return await ctx.send(embed=embed)
                embed.description += f"\n{i}: {val}"
    ...
    async def alias_remove(self, ctx, *, name: str.lower):
        ...
        if name in self.bot.aliases:
            ...
            embed = discord.Embed(
                title=_("Removed alias"),
                color=self.bot.main_color,
                description=_("Successfully deleted `{name}`.").format(name=name),
            )
        else:
            embed = utils.create_not_found_embed(name, self.bot.aliases.keys(), "Alias")
        ...
        if len(values) == 1:
            ...
            if not self.bot.get_command(linked_command):
                ...
                if alias_command is not None:
                    ...
                    return await ctx.send(embed=embed)
            embed = discord.Embed(
                title=_("Edited alias"),
                color=self.bot.main_color,
                description=_('`{name}` now points to: "{value}".').format(name=name, value=values[0]),
            )
    ...
    async def permissions_add(
        ...
        *,
        ...
        embed = discord.Embed(
            title=_("Success"),
            color=self.bot.main_color,
            description=_("Permission for `{name}` was successfully updated.").format(name=name),
        )
        return await ctx.send(embed=embed)
    ...
    def _get_perm(self, ctx, name, type_):
        ...
        if not permissions:
            embed = discord.Embed(
                title=_("Permission entries for {type} `{name}`:").format(type=type_, name=name),
                description=_("No permission entries found."),
                color=self.bot.main_color,
            )
        else:
            values = []
        ...
        *,
        ...
        if name is None and user_or_role not in {"command", "level", "override"}:
            ...
            desc_cmd = (
                ...
                if cmds
                else _("No permission entries found.")
            )
            desc_level = (
                    ...
                    if not overrides:
                        ...
                        for items in zip_longest(
                            *(iter(sorted(overrides.items())),) * 15
                            ...
                            embed.set_author(
                                name=_("Permission Overrides"), icon_url=ctx.guild.icon_url
                            )
                            embeds.append(embed)
