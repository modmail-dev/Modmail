...
class ModmailApiClient(UserClient, ApiClient):
    ...
    async def create_log_entry(self, recipient=None,
                               channel=None,
                               creator=None, *, payload=None):
    ...
    async def post_log(self, channel_id, data,  *, force=False):
        params = '?force=1' if force else ''

        return await self.request(self.LOGS + f'/{channel_id}{params}',
                                  method='POST',
                                  payload=data)
